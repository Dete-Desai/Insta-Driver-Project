...
import {
  ...
  employerGetJobPosts,
  stopAndArchiveJobAd,
} from "../../../../redux/actions/employer_actions/job_post_actions";
import ActiveJobsTable from "./ActiveJobsTable";
import InActiveJobsTable from "./InActiveJobsTable";

const drivingJobsSelector = (state) => {
  const allJobs = state.employer_reducers.job_posts;
  if (!allJobs) return [];

  var drivingJobs = allJobs.filter((job) => job.adType === "Driving Jobs");
  return drivingJobs;
};

export const getDaysElapsed = (startDate, endDate) => {
...
export const isExpired = (jobItem) => {
  //early out
...
export const ManagePosts1 = () => {
  const dispatch = useDispatch();
  const token = useSelector(
    (state) => state.user_reducers.user_credentials.token
  );
  const allJobs = useSelector((state) => state.employer_reducers.job_posts);
  const drivingJobs = useSelector(drivingJobsSelector);
  const activeDrivingJobs = drivingJobs.filter((job) => !isExpired(job));
  const inActiveDrivingJobs = drivingJobs.filter((job) => isExpired(job));
  ...
  const shownondrivingjobs = () => {
    let route = "employer-manage-posts-2";
    history.push(route);
  };

  const onView = (id) => {
    let route = `employer-job-card-ad/${id}`;
    history.push(route);
  };

  const onDelete = (id) => {
    dispatch(deleteJobAd(token, id));
  };

  const onStopAndArchive = (id) => {
    dispatch(stopAndArchiveJobAd(token, id));
  };
                ...
                <div className="container-fluid">
                  <div className="container">
                    <div className="row">
                      {active && (
                        <ActiveJobsTable
                          jobs={activeDrivingJobs}
                          onStopAndArchive={onStopAndArchive}
                          onView={onView}
                        />
                      )}
                      {!active && (
                        <InActiveJobsTable
                          jobs={inActiveDrivingJobs}
                          onView={onView}
                          onDelete={onDelete}
                        />
                      )}
                    ...
                    <div id="all_job_ads">
                      ...
                      >
                        ...
                        <ActiveJobsTable jobs={drivingJobs} />
                        <br />
                      ...
                      >
                        ...
                        <InActiveJobsTable jobs={inActiveDrivingJobs} />
                      </div>
                      <div
...
ManagePosts1.propTypes = {};

export default ManagePosts1;
