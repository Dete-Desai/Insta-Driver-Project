...
import { isExpired } from "./ManagePosts1";
import InActiveJobsTable from "./InActiveJobsTable";
import ActiveJobsTable from "./ActiveJobsTable";

const nonDrivingJobsSelector = (state) => {
  const allJobs = state.employer_reducers.job_posts;
  if (!allJobs) return [];

  var nonDrivingJobs = allJobs.filter(
    (job) => job.adType === "Non-driving Jobs"
  );
  return nonDrivingJobs;
};

export const ManagePosts2 = () => {
  ...
  const nonDrivingJobs = useSelector(nonDrivingJobsSelector);
  const activeNonDrivingJobs = nonDrivingJobs.filter((job) => !isExpired(job));
  const inActiveNonDrivingJobs = nonDrivingJobs.filter((job) => isExpired(job));
                  ...
                  <div className="container">
                    ...
                    <div id="all_job_ads">
                      ...
                      >
                        ...
                        <InActiveJobsTable jobs={nonDrivingJobs} />
