...
function get_search_message(category_id, s_type, location, flag) {
  if (category_id > 0 && s_type !== "") {
    $.get(
      SITEROOT + "home/get_search_message",
      { category_id: category_id, s_type: s_type, location: location },
      function (data) {
        if ($.trim(data)) {
          if (flag === 1 && s_type !== "") {
            $("#sr_msg_nolist")
              .html(data.html)
              .addClass("alert alert-dark sr_msg_nolist");
          } else if (flag === 2 && s_type !== "") {
            $("#sr_msg_list")
              .html(data.html)
              .addClass("alert alert-dark sr_msg_list");
          }
